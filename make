...
for mdpath in $(cd "$root" && find . -type f -name '*.md')
do
	mdpath="${mdpath#./}"
...
for f in fonts.css style.css script.js
do
	if [ ! -e "$build/public/$f" ]
	then
...
for svg in "$src"/icon/*.svg
do
	./scripts/genicons "$svg" "$build/public"

	if [ ! -e "$build/public/$(basename $svg)" ]
	then
		cp "$svg" "$build/public"
